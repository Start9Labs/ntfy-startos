...
This repo packages [ntfy](https://github.com/binwiederhier/ntfy) for StartOS.
...
NTFY runs the upstream `binwiederhier/ntfy` Docker image. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `binwiederhier/ntfy:v<new version>`.
