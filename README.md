...
**Yours** — everything the Configure action exposes: `base-url`, self-registration, the three attachment limits, the per-user daily bandwidth budget, message retention, the VAPID contact email, and the log level. Leaving a field blank removes the key, which hands the setting back to ntfy's own default.

**Anything else** — the shape is a `z.looseObject`, so a key the package does not model survives every write. A setting ntfy supports but Configure does not can be added to the file by hand.
...
Every general server setting: base URL, self-registration, attachment limits, the per-user daily bandwidth budget, message retention, VAPID contact email, and log level.
...
Neither permission field prefills from the stored grant. The grant being edited is keyed on the topic chosen in the same form, so there is no single current value to show; both fields open at their declared default and carry a footnote saying so. The authoritative view is the grant list returned after applying.
